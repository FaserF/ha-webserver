...
def main():
    ...
    repo = os.environ.get("REPO", "faserf/ha-webserver").lower()
    ...
    for path in ["custom_components/webserver_app/manifest.json", "pyproject.toml"]:
        try:
    ...
    for f in changed_files:
        if f.startswith("custom_components/webserver_app/translations/"):
            translation_count += 1
    ...
    body_parts = [
        f"# Webserver App {version}  {channel_badge}",
        "",
        prerelease_note,
        "## 📋 What's Changed",
        "",
